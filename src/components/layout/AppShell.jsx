  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 lg:grid lg:grid-cols-[280px_1fr]">
      <aside className="hidden min-h-screen bg-slate-950 p-5 text-white lg:flex lg:flex-col lg:sticky lg:top-0 lg:h-screen">
        <NavLink to={basePath || '/'} className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-4">
        ...
        <div className="mt-auto rounded-2xl border border-slate-800 bg-slate-900 p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0"><p className="truncate text-sm font-bold">{name}</p><p className="truncate text-xs text-slate-400">{!readOnly && profile?.role === 'admin' ? 'Administrator' : 'Student'}</p></div>
          </div>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur lg:hidden">
          <div className="flex items-center justify-between gap-3">
            <NavLink to={basePath || '/'} className="flex items-center gap-2 font-black"><img src={hiveLogoUrl} alt="" className="h-8 w-8 rounded-lg object-contain" /> THE HIVE</NavLink>
        ...
        <main className="min-h-screen">{readOnly && <div className="flex flex-wrap items-center justify-between gap-3 border-b border-amber-200 bg-amber-50 px-6 py-4"><div><p className="font-black">Student view: {name}</p><p className="text-sm text-slate-600">Read-only preview. The student’s progress and attempts will not change.</p></div><div className="flex flex-wrap gap-3"><Link to="/admin/student-view" className="hive-secondary-button text-sm">Change student</Link><Link to="/admin" className="hive-primary-button text-sm">Return to admin</Link></div></div>}<Outlet /></main>
